#!/usr/bin/env python3
"""Build the browser-friendly story neighbor index from TensorFlow projector data."""
...
def main():
    ...
    for author in graph["nodes"]:
        for story_id, title in author.get("stories", {}).items():
            by_title[normalize(title)].append((story_id, normalize(author["id"])))
    ...
    for row_number, row in enumerate(metadata):
        candidates = [candidate for candidate in by_title[normalize(row["Título"])] if candidate[0] not in used]
        if not candidates:
            continue
        metadata_author = normalize(row["Autor"])
        matching = [candidate for candidate in candidates if all(part in metadata_author for part in candidate[1].split())]
    ...
    tensors = np.loadtxt(ROOT / "tensors_generator/stories_tensors.tsv", dtype=np.float32, delimiter="\t")
    ...
    for start in range(0, len(mapped_rows), batch_size):
        similarities = mapped_tensors[start : start + batch_size] @ mapped_tensors.T
    ...
    # A catalog story can occasionally be newer than the checked-in tensor. Give
    # it the consensus recommendations of its author's embedded stories until the
    # TensorFlow export is regenerated, so every story page remains functional.
    for author in graph["nodes"]:
        mapped_siblings = [story_id for story_id in author.get("stories", {}) if story_id in neighbors]
        for story_id in author.get("stories", {}):
            ...
            for sibling_id in mapped_siblings:
                for neighbor in neighbors[sibling_id]:
                    ...
                    if neighbor_id != story_id and neighbor_id not in [entry["id"] for entry in consensus]:
            ...
            if story_file.exists():
                ...
                story_metadata[story_id] = {
                    "readingTime": float(story_data.get("metadata", {}).get("reading_time_min", 0))
                }
    ...
    (ROOT / "static/storyReaderMetadata.json").write_text(
        json.dumps(story_metadata, ensure_ascii=False, separators=(",", ":")) + "\n"
    )
    print(f"Wrote {len(neighbors)} mapped stories to {output.relative_to(ROOT)}")
